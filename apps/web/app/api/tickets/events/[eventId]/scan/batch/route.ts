...
import { scanTicketBatch } from "@/lib/ticketing/scanner";

export const runtime = "nodejs";

/**
 * Legen van de wachtrij die de scanner offline heeft opgebouwd. Honderd scans van
 * elk hoogstens twee kilobyte passen ruim in de limiet hieronder.
 */
export async function POST(
  ...
  try {
    ...
    return Response.json(
      await scanTicketBatch(eventId, await readLimitedJson(request, 512 * 1024)),
    );
  } catch (error) {
    ...
    console.error("Ticket scan batch failed", error);
