...
| `/admin/fakscanner` (bar check-ins) | `fakscanner.manage` | Per working year (`?jaar=`): the points ranking (`FakCheckin`), the settings (double-count window, points per free beer, bar-day rollover) and the scan log (`FakScanLog`, incl. failures). |
