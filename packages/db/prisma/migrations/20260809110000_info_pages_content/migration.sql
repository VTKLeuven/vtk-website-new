-- De Info-tab had vier menu-items waarvan er twee hetzelfde ding waren: de
-- pagina "Reservaties en logistiek" en een kale link "Uitleendienst" naar de
-- logistiek-app. Die link is hier weer weg (hij kwam uit de migratie
-- `20260808151000_uitleendienst_header_link`); de pagina zelf heet nu
-- Uitleendienst en draagt een knop naar de app.
--
-- Meteen ook de inhoud van Shiften en Uitleendienst ingevuld. Die stond nog op
-- de prototypetekst uit de seed ("Voor het prototype verwijzen we naar ...")
-- terwijl vtk.be er wel echte pagina's voor heeft: Uitleendienst, en Medewerkers
-- voor het shiftverhaal.
--
-- De seed (`packages/db/prisma/seed.ts`) draagt dezelfde teksten, want die is
-- wat een verse database krijgt; deze migratie is er voor de databases die al
-- bestaan, want de seed draait niet bij een deploy.

-- 1. De dubbele menulink weg. Enkel deze ene URL, want de andere items onder
--    Info (piano) moeten blijven staan.
DELETE FROM "HeaderTabLink" l
USING "HeaderTab" t
WHERE l."tabId" = t."id"
  AND t."code" = 'AANBOD'
  AND l."url" IN ('https://logistiek.vtk.be', 'https://logistiek.dev.vtk.be');

-- 2. De pagina heet voortaan Uitleendienst, ook in het adres. Dat repareert
--    meteen de redirect van het oude vtk.be/nl/page/uitleendienst: die komt uit
--    op /p/uitleendienst en gaf 404 zolang de slug anders was.
--    Wijkt uit wanneer er al een pagina op die slug staat, want slug is uniek en
--    een migratie mag een deploy niet halverwege afbreken.
UPDATE "Page" p
SET
  "slug" = 'uitleendienst',
  "titleNl" = 'Uitleendienst',
  "titleEn" = 'Equipment service'
WHERE p."slug" = 'reservaties-en-logistiek'
  AND NOT EXISTS (SELECT 1 FROM "Page" o WHERE o."slug" = 'uitleendienst');

-- 3. Inhoud, samenvatting en knop. Enkel voor pagina's waarvan de inhoud nog
--    nooit door iemand opgeslagen is (`contentEditedAt IS NULL`): wie er in de
--    editor al aan gewerkt heeft, mag dat niet door een deploy kwijtspelen.
--    `needsYearlyEdit` gaat aan omdat er bedragen, kaarten en documenten in
--    staan die per werkingsjaar veranderen.
UPDATE "Page"
SET
  ...
  "contentMdNl" = $md$VTK leent materiaal en vervoer uit aan haar leden en werkgroepen. Reserveren doe je online met je VTK-account. Vragen mag altijd naar <logistiek@vtk.be>.
...
Wie materiaal of vervoer reserveert, gaat akkoord met de [uitleenvoorwaarden](https://drive.google.com/file/d/1B-W9ztC9uUO6WQdCAE-X8hO7bcjCitwF/view). Je bent verantwoordelijk voor wat je meeneemt: breng het terug zoals je het kreeg, en meld schade meteen in plaats van achteraf.$md$,
  "contentMdEn" = $md$VTK lends equipment and transport to its members and work groups. You reserve online with your VTK account. Questions are always welcome at <logistiek@vtk.be>.
...
Anyone reserving equipment or transport agrees to the [lending conditions](https://drive.google.com/file/d/1B-W9ztC9uUO6WQdCAE-X8hO7bcjCitwF/view). You are responsible for what you take with you: return it as you received it, and report damage straight away rather than afterwards.$md$
WHERE "slug" = 'uitleendienst'
  AND "contentEditedAt" IS NULL;
...
SET
  ...
  "ctaUrl" = '/shift',
  "needsYearlyEdit" = true,
  "contentMdNl" = $md$VTK draait op vrijwilligers. Achter elke cantus, elk feest, elke broodjesbalie en elke boekenverkoop staan leden die een shift opnemen. Je kiest zelf welke shift je doet, en je bent tot niets verplicht.
...
Vragen over shiften, problemen met de werklijsten, of wil je op de medewerkerslijst? Laat het de vice weten. Wie op die lijst staat, krijgt wekelijks een overzicht van de activiteiten en de openstaande shiften. Ook dat houdt geen verplichting in.$md$,
  "contentMdEn" = $md$VTK runs on volunteers. Behind every cantus, party, sandwich counter and book sale are members who take a shift. You pick the shifts you want, and you commit to nothing.
...
Questions about shifts, trouble with the sign-up lists, or do you want to be on the volunteers' mailing list? Let the vice know. Everyone on that list gets a weekly overview of activities and open shifts. That, too, commits you to nothing.$md$
WHERE "slug" = 'shiften'
  AND "contentEditedAt" IS NULL;
