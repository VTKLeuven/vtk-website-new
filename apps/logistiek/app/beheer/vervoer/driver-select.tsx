 ...
 *
 * Vraagt het voertuig een karchauffeur (`needsVanDriver`), dan wint dat
...
export function DriverOptions({
  ...
  if (needsVanDriver) {
    const withVan = drivers.filter((driver) => driver.canDriveVan);
    const without = drivers.filter((driver) => !driver.canDriveVan);
    return (
      <>
        {withVan.length > 0 ? (
