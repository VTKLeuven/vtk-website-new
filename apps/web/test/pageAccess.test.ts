...
function session(opts: { isSuperAdmin?: boolean; permissions?: string[]; roleIds?: string[] }): SessionPayload {
  return {
...
describe('losesOwnPageAccess', () => {
  it('waarschuwt wanneer geen van de overblijvende rollen van hem is', () => {
    expect(losesOwnPageAccess({ canEditAll: false, myRoleIds: ['cudi'], nextRoleIds: ['theokot'] })).toBe(true);
  });

  it('waarschuwt wanneer alle rollen weg zijn', () => {
    expect(losesOwnPageAccess({ canEditAll: false, myRoleIds: ['cudi'], nextRoleIds: [] })).toBe(true);
  });
  ...
  it('zwijgt voor editAll/superadmin: die verliezen niets', () => {
    expect(losesOwnPageAccess({ canEditAll: true, myRoleIds: ['cudi'], nextRoleIds: [] })).toBe(false);
  });
