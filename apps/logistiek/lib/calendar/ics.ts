/**
 * iCalendar (RFC 5545) serialisatie voor de agendafeed van de transportplanning.
 *
 * Bewust zonder afhankelijkheden en zonder DB: elk item is precies één VEVENT en
 * de hele generator past in dit bestand. Dat maakt hem ook volledig
 * unit-testbaar (test/ics.test.ts).
 *
 * **Dit is een kopie van `apps/web/lib/calendar/ics.ts`**, en dat is een
 * afweging geweest. Hoisten naar een gedeeld pakket is netter, maar een nieuw
 * workspace-pakket vraagt een volledige lockfile-regeneratie (zie AGENTS.md), en
 * die laat `better-auth` doorfloaten naar een versie waarop `packages/auth` niet
 * meer typecheckt. De generator is klein, afhankelijkheidsvrij en getest; de
 * kopie kost minder dan die val. Wijzig je hier iets aan de RFC-kant (vouwen,
 * escapen, DTSTAMP), kijk dan of het daar ook moet.
 */
