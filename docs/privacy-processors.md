...
| Google Workspace (Google Ireland Ltd) | Group addresses of the association (`activiteiten@vtk.be`): the site writes the `@vtk.be` address of each member of a linked post into the matching Google group, and reads the directory (id, address, name) to link accounts | `apps/web/lib/google/`, `apps/web/app/api/google/maintenance/`, `Setting` key `google.config` | Only active when the service account is configured. Record the Workspace DPA and region, keep domain-wide delegation limited to the scopes in `lib/google/client.ts`, and note that the sync only ever touches groups that have a `MailGroup` row |
...
| Bancontact Company | Direct app payments (Bancontact Pro; formerly Payconiq by Bancontact): order reference, amount, status and payment identifiers. No buyer email is sent, unlike Mollie | `packages/payments/src/bancontact.ts`, `apps/web/app/api/tickets/bancontact/webhook` | Separate merchant contract and DPA from Mollie; keep the API key out of the fixtures and out of `Setting`, and document that this is a second payment processor alongside Mollie |
...
| Umami, self-hosted by VTK (no third party) | Consent-gated, cookieless page-view statistics and Core Web Vitals performance metrics: page path, referrer, screen size, browser language, performance metrics (LCP, INP, CLS, FCP, TTFB) and a daily-rotating hash of IP address plus user agent; the IP address itself is not stored | `apps/web/lib/analytics.ts`, `apps/web/app/layout.tsx`, `infra/docker-compose.yml` (`umami`, `umami-db-init`), `.env.example` (`UMAMI_*`) | Runs on VTK's own server and database, so there is no recipient to sign a DPA with; the hosting and PostgreSQL row already covers where it lives. Keep `DISABLE_TELEMETRY=1` so the instance itself calls nobody, limit dashboard accounts, set a retention window and never point `UMAMI_PUBLIC_URL` at a hosted Umami |
